 ...
 *
 * В файле каждое правило записано одной строкой: «название — пояснение» со
 * строчной буквы после тире. Держим так же, поэтому на пару «заголовок и
 * абзац» строку не разводим: название выделено начертанием внутри неё.
 */
const PROTECTION: { title: string; text: string }[] = [
  ...
  { title: "Аудит и\u00A0мониторинг", text: "все действия логируются" },
];
...
const FAQ: FaqItem[] = [
  ...
  },
  {
    ...
    answer: (
      <p>
        При гибридной схеме в&nbsp;облако передаётся по&nbsp;зашифрованному
        каналу (TLS/mTLS/IPSec) вектор запроса. Данные размещаются в&nbsp;вашем
        контуре. После генерации ответа облачный сегмент не&nbsp;сохраняет
        историю запроса.
      </p>
    ),
  },
    ...
    <>
      ...
      <section className="w-full bg-bg-page py-48 md:py-[100px]">
        <div className="container-page flex flex-col gap-24 md:gap-[56px]">
          ...
          {/*
            Правила — линия сверху, как в блоке безопасности на /ai-platform.
            Отступ справа и увеличенный зазор только от md: в одну колонку они
            обрезали линию на 24 и разносили правила так, что подпись переставала
            читаться вместе со своей линией.
          */}
          <div className="grid gap-16 md:grid-cols-2 md:gap-24 lg:grid-cols-4">
            {PROTECTION.map((rule) => (
              <article
                key={rule.title}
                className="flex flex-col gap-[10px] pt-[20px] md:pr-24"
              >
                <span aria-hidden className="h-px w-full bg-text-primary" />
                <p className="text-body-m text-text-secondary">
                  <b className="text-body-l font-medium text-text-primary">
                    {rule.title}
                  </b>{" "}
                  — {rule.text}
                </p>
