          ...
          {items.map((benefit) => (
            <article
              ...
              className={`flex flex-1 flex-col gap-16 overflow-hidden rounded-[24px] px-40 pt-40 pb-24 ${
                /*
                  345 из макета — под карточку с тегами внизу. Именно
                  минимальная высота, а не жёсткая: в макете в ряду три
                  карточки по 373, а у бухгалтерии их четыре, по 282 — длинный
                  заголовок в три строки перестаёт помещаться, и при `h-[345px]`
                  теги уезжали за нижний край и срезались `overflow-hidden`.
                  С `min-h` ряд по-прежнему выравнивается по 345, но растёт,
                  если содержимому не хватает места (все карточки в ряду тянутся
                  до высоты самой высокой).

                  Без тегов такая высота оставляла половину карточки пустой,
                  поэтому ряд выравнивается по содержимому.
                */
                hasTags ? "md:min-h-[345px]" : "md:min-h-[220px]"
              } ${CARD_GRADIENT}`}
            >
